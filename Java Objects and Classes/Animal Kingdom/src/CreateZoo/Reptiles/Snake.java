...
public class Snake extends Animal implements AnimalSurvivalNeeds, Reptile {
    ...
    public void reproduce() {
        if (isAddedToZoo()){
            if (AddAnimalToZoo.getNumberOfAnimals(this) > 1 && hasFood() && hasSleep()){
                ...
                print(getName() + " needs a mate to reproduce. Please add another Snake to the zoo first.");
            }
        } else {
    ...
    public void socialize() {
        if (isAddedToZoo()){
            if (AddAnimalToZoo.getNumberOfAnimals(this) > 1 && hasSleep() && hasFood()){
                useFood(20);
                useSleep(15);
                print(getName() + " is now socializing with other Snakes.");
            } else if (AddAnimalToZoo.getNumberOfAnimals(this) > 1 && (!hasSleep() || !hasFood())){
                print(getName() + " needs to sleep or eat first.");
            } else if (AddAnimalToZoo.getNumberOfAnimals(this) <= 1){
                print(getName() + " needs some friends to socialize. Please add more Snakes to the zoo first.");
            }
        } else {
